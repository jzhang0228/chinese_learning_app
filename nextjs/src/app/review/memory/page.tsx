...
export default function ReviewMemoryPage() {
  ...
  const [pairs, setPairs] = useState<{ chinese: string; english: string }[] | null>(null);
  ...
  const fetchAndSetup = async () => {
    try {
      ...
      const words: Word[] = data.words || data;
      ...
      setPairs(selected.map((w) => ({ chinese: w.chinese, english: w.english })));
      setWon(false);
    } catch {
